...
class SignalingServer extends Command
{
    ...
    protected $description = 'Start the WebRTC signaling server (auto-detects SSL for WSS)';
    ...
    public function handle(): void
    {
        ...
        if ($hasSSL) {
            ...
            $ws->transport = 'ssl';
            $this->info("WSS (Secure) signaling server starting on port {$port}...");
        } else {
            $ws = new Worker("websocket://0.0.0.0:{$port}");
            $this->info("WS signaling server starting on port {$port}...");
